...
def get_spectrograms(sound_file, sr=SAMPLE_RATE, fft_size=FFT_SIZE):
    ...
    """
    y, _ = librosa.load(sound_file, sr=sr)
    y_fft = librosa.stft(y, n_fft=fft_size, hop_length=HOP_LENGTH, win_length=WIN_LENGTH)
    return np.abs(y_fft.transpose(1, 0))
...
def read_bin(file_path):
    ...
    with h5py.File(file_path, 'r') as f:
        data = np.array(f['mag_sgram'])
    return {'mag_sgram': np.expand_dims(data, axis=0)}


def pad_zeros(array, reference_shape):
...
def pad_same(array, reference_shape):
    """
    Pad with same data to fit array to reference_shape
    """
    result = np.zeros(reference_shape)
    ind = 0
    while reference_shape[1] > array.shape[1]:
        result[:, ind:ind + array.shape[1], :] = array
        ind += array.shape[1]
        reference_shape[1] -= array.shape[1]
    result[:, ind:, :] = array[:, :reference_shape[1], :]
    return result


def data_generator(file_list, bin_root, frame=True, batch_size=1, use_zeros_to_pad=True):
    ...
    """
    pad = pad_zeros if use_zeros_to_pad else pad_same
    index = 0
    while True:
